...
function navigateTo(id) {
  const btn = [...document.querySelectorAll('nav button')].find(b => {const oc = b.getAttribute('onclick');return oc && oc.includes(`'${id}'`);});
...
function showUtilTab(tab, btn) {
  ...
  document.getElementById('utilTabCandito').style.display      = tab === 'candito'      ? 'block' : 'none';
  document.getElementById('utilTabPropoli').style.display      = tab === 'propoli'      ? 'block' : 'none';
